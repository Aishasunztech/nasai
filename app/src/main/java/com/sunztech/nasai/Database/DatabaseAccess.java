package com.sunztech.nasai.Database;
...
import com.sunztech.nasai.Models.BookChapter;
import com.sunztech.nasai.Models.HadithItem;
...
public class DatabaseAccess {
    ...
    public ArrayList<HadithItem> getChapterHadith(int chapterId) {
        ...
        c = db.rawQuery("select * from tbl_Nasai WHERE Kitab_ID = " + chapterId, new String[]{});
    ...
    public void updateBookMark(HadithItem hadithItem)
    {
        ...
        db.update("tbl_Nasai", args, "hadees_number = " + hadithItem.getHadees_number(), null);
    }

    public ArrayList<HadithItem> getBookMarks()
    {
        HadithItem hadees = null;
        c = db.rawQuery("select * from tbl_Nasai WHERE is_bookmarked = " + 1, new String[]{});
        ArrayList<HadithItem> hadeesItems = new ArrayList<>();
        if (c != null) {
            while (c.moveToNext()) {
    ...
    public ArrayList<HadithItem> searchHadith(String columnName, String searchingValue) {
        ...
        if(columnName.equals("hadees_number")) {
            c = db.rawQuery("select * from tbl_Nasai WHERE " + columnName + " = " + searchingValue, new String[]{});

        }
        else{
            c = db.rawQuery("select * from tbl_Nasai WHERE "  + columnName + " like '%" + searchingValue + "%'", new String[]{});
